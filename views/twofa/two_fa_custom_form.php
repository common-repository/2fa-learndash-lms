    ...
    <form name="form_custom_form_config" method="post" action="" id="mo2f_custom_form_config">
		...
		if ( $isRegistered == 'false' ) {
            ...
            <div style="padding: 10px;border: red 1px solid">
                <a href="admin.php?page=molms_account"> Register/Login</a> with miniOrange to Enable the Shortcode
            </div>
			<?php
		}
		?>
        <h2> Shortcode </h2>
        <hr>
        <p style="color: red">*Add this on the page where you have your registration form to enable OTP verification for
            the same.</p>
        ...
        <table>
            ...
            <tr>
                <td>
                    <input type="checkbox" name="mo2f_method_phone" id="mo2f_method_phone"
                           value="phone" <?php if ( get_site_option( 'mo2f_custom_auth_type' ) == 'phone' or get_site_option( 'mo2f_custom_auth_type' ) == 'both' ) {
						echo "checked";
					} ?> >
                    <label for="mo2f_method_phone"> OTP over SMS </label>
                </td>
                <td>
                    <input type="checkbox" name="mo2f_method_email" id="mo2f_method_email"
                           value="email" <?php if ( get_site_option( 'mo2f_custom_auth_type' ) == 'email' or get_site_option( 'mo2f_custom_auth_type' ) == 'both' ) {
						echo "checked";
					} ?>>
        ...
        <table style="padding: 10px;">
            ...
            <tr>
				...
				if ( $isRegistered != 'false' ) {
				?>
                <td style="padding: 5px;">SMS Transactions: <strong><?php echo esc_html( $SMSTransactions ); ?></strong>
                </td>
                <td></td>
                <td style="padding: 5px;">Email Transactions:
                    <strong><?php echo esc_html( $EmailTransactions ); ?></strong></td>
            </tr>
            <tr>
                <td>
                    <p style="color:red">*You will get 10 SMS and 30 EMAIL Transactions in Free Account, to Recharge <a
                                href="<?php echo esc_url_raw( MOLMS_HOST_NAME ) . '/moas/login?redirectUrl=https://login.xecurify.com/moas/initializepayment&requestOrigin=otp_recharge_plan' ?>"
        ...
        <div style="padding: 20px;border: 1px #DCDCDC solid">
            <table>
                ...
                <tr>
                    <td>
                        <div class="button" style="<?php if ( $is_woocommerce ) {
							echo 'color : white; background-color: teal';
						} ?>" name="wc_auto" id="wc_auto"> WooCommerce
                        </div>
                    </td>
                    <td>
                        <div class="button" style="<?php if ( $is_bbpress ) {
							echo 'color:white; background-color: teal';
						} ?>" name="bbpress_auto" id="bbpress_auto">BB Press
                        </div>
                    </td>
                    <td>
                        <div class="button" style="<?php if ( $is_custom ) {
							echo 'color:white; background-color: teal';
						} ?>" name="custom_auto" id="custom_auto">Custom
                        </div>
                    </td>
                </tr>
                </tbody>
            </table>
            <h4 id="enterMessage" name="enterMessage"
                style="display: none;padding:8px; color: white; background-color: teal">Enter Selectors for your
                Form</h4>
            <h3>Form Selector<span style="color: red;font-size: 14px">*</span></h3>
            <input type="text" value="<?php echo esc_html( get_site_option( 'mo2f_custom_form_name' ) ); ?>"
                   style="width: 100%" name="mo2f_shortcode_form_selector" id="mo2f_shortcode_form_selector"
                   placeholder="Example #form_id" <?php if ( $is_any_of_woo_bb ) {
				echo 'disabled';
			} ?> >
            ...
            <input type="text" style="width: 100%"
                   value="<?php echo esc_html( get_site_option( 'mo2f_custom_email_selector' ) ); ?>"
                   name="mo2f_shortcode_email_selector" id="mo2f_shortcode_email_selector"
                   placeholder="example #email_field_id" <?php if ( $is_any_of_woo_bb ) {
				echo 'disabled';
			} ?> >
            ...
            <input type="text" style="width: 100%"
                   value="<?php echo esc_html( get_site_option( 'mo2f_custom_phone_selector' ) ); ?>"
                   name="mo2f_shortcode_phone_selector" id="mo2f_shortcode_phone_selector"
                   placeholder="example #phone_field_id">

            <h3>Submit Button Selector <span style="color: red;font-size: 14px">*</span></h3>
            <h4>Enter selector for Submit Button</h4>
            <input type="text" style="width: 100%"
                   value="<?php echo esc_html( get_site_option( 'mo2f_custom_submit_selector' ) ); ?>"
                   name="mo2f_shortcode_submit_selector" id="mo2f_shortcode_submit_selector"
                   placeholder="example #submit_button_id" <?php if ( $is_any_of_woo_bb ) {
				echo 'disabled';
			} ?> >
...
<div class="mo_lms_setting_layout">
    ...
    <p>We support most of the login forms present on the wordpress. And our plugin is tested with almost all the forms
        like Woocommerce, Ultimate Member, Restrict Content Pro and so on.</p>
    ...
    <div>
        <table class="customloginform" style="width: 100%;" align="left">
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/woocommerce.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit; padding-right: 50px;">
                        Woocommerce</h3>
                </td>
                <td style="align-items: right;">
                    <form id="woocommerce_login_prompt_form" method="post">
                        <div align="center">
                            <input type="hidden" name="molms_woocommerce_login_prompt_nonce"
                                   value="<?php echo esc_attr( wp_create_nonce( "molms-woocommerce-login-prompt-nonce" ) ) ?>"/>
                            <input type="checkbox" name="woocommerce_login_prompt"
                                   onchange="document.getElementById('woocommerce_login_prompt_form').submit();" <?php if ( get_site_option( 'mo2f_woocommerce_login_prompt' ) ) {
								?> checked <?php
							} ?> <?php if ( ! molms_Utility::get_mo2f_db_option( 'mo2f_enable_2fa_prompt_on_login_page', 'site_option' ) ) {
								?> checked <?php
							} ?>/>
                        </div>
                ...
                <td>
                    <div style="text-align: center;">
                        <a href='<?php echo esc_url_raw( $two_factor_premium_doc['Woocommerce'] ); ?>'
                           target="blank"><span class="dashicons dashicons-text-page mo2f_doc_icon_style"
                                                style="font-size: 25px;color: #199C95"></span></a>
                    </div>
                </td>
            </tr>
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/ultimate_member.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">Ultimate
                        Member</h3>
                </td>
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/restrict_content_pro.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">Restrict Content
                        Pro</h3>
                </td>
                <td style="text-align: center;">
                    <input type="checkbox" name="" checked>
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/theme_my_login.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">My Theme
                        Login</h3>
                </td>
                <td style="text-align: center;">
                    <input type="checkbox" name="" checked>
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/user_registration.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">User
                        Registration</h3>
                </td>
                <td style="text-align: center;">
                    <input type="checkbox" name="" checked>
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/Custom_Login_Page_Customizer_LoginPress.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">Custom Login Page
                        Customizer | LoginPress</h3>
                </td>
                <td style="text-align: center;">
                    <input type="checkbox" name="" checked>
                </td>
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;float: left;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/Admin_Custom_Login.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">Admin Custom
                        Login</h3>
                </td>
                <td style="text-align: center;">
                    <input type="checkbox" name="" checked>
            ...
            <tr>
                <td>
					<?php echo '<img style="width:30px; height:30px;display: inline;float: left;" src="' . dirname( plugin_dir_url( dirname( __FILE__ ) ) ) . '/includes/images/RegistrationMagic_Custom_Registration_Forms_and_User_Login.png">'; ?>
                    <h3 style="margin-left: 15px; font-size: large; display: inline; float: inherit;">RegistrationMagic
                        – Custom Registration Forms and User Login</h3>
                </td>
                <td style="text-align: center; ">
                    <input type="checkbox" name="" checked>
                </td>
    ...
    <div style="float: left;">
        <br>
        <b style="color: red; ">**If you want to enable/disable 2FA prompt on other Custom login pages please Contact
            us.</b>
        <br>
        <b style="color: red;">**This feature will only work when you enable 2FA prompt on wordpress login
            page.</li></b>

        <p style="font-size:15px">If there is any custom login form where Two Factor is not initiated for you, plese
            reach out to us by dropping a query in the <b>Support</b> section.</p>
    </div>
</div>