...
$mo2f_second_factor = molms_get_activated_second_factor( $user );
...
if ( $mo2f_second_factor == 'GOOGLE AUTHENTICATOR' ) {
	...
	$selectedMethod = molms_decode_2_factor( $mo2f_second_factor, "servertowpdb" );
	$testMethod     = $selectedMethod;
}
...
if ( $selectedMethod != 'NONE' and ! MOLMS_IS_ONPREM ) {
...
if ( get_user_meta( $user->ID, 'configure_2FA', true ) ) {
	...
	echo '<div class="mo_lms_setting_layout">';
	molms_show_2FA_configuration_screen( $user, $current_selected_method );
	echo '</div>';
} elseif ( get_user_meta( $user->ID, 'test_2FA', true ) ) {
	$current_selected_method = get_user_meta( $user->ID, 'mo2f_2FA_method_to_test', true );
	echo '<div class="mo_lms_setting_layout">';
	molms_show_2FA_test_screen( $user, $current_selected_method );
	echo '</div>';
} elseif ( get_user_meta( $user->ID, 'register_account_popup', true ) && $can_display_admin_features ) {
	molms_display_customer_registration_forms( $user );
} else {
	...
	$showOTP                            = false;
	if ( MOLMS_IS_ONPREM ) {
		$selectedMethod         = $molms_db_queries->get_user_detail( 'mo2f_configured_2FA_method', $user->ID );
		...
		if ( $selectedMethod == "Google Authenticator" ) {
			...
			$i         = get_site_option( 'mo2f_time_slice', 0 );
			$otpcode   = $gauth_obj->getCode( $secret, $currentTimeSlice + $i );
			$showOTP   = true;
		}
	} ?>
    <div id="lms_message"></div>
        ...
        <div>
            <div>
                <a class="mo2f_view_free_plan_auth_methods" onclick="show_free_plan_auth_methods()">
                    ...
                    <p class="mo2f_heading_style"
                       style="padding:0px;"><?php echo esc_html( molms_lt( 'Authentication methods' ) ); ?>
						<?php if ( $can_display_admin_features ) { ?>
                            <span style="color:limegreen">( <?php echo esc_html( molms_lt( 'Current Plan' ) ); ?> )</span>
						<?php } ?>
						<?php if ( $showOTP ) { ?>
                            <span style="color:black">[ <?php echo esc_html( molms_lt( 'Current OTP: ' ) ) . esc_html( $otpcode ); ?> (<span
                                        style="color:blue" onclick="window.location.reload();">Refresh</span>)] </span>
						<?php } ?>

                        <button class="btn btn-primary btn-large" id="test"
                                style="float:right; margin-right: 20px; height: 36px"
                                onclick="testAuthenticationMethod('<?php echo esc_html( $selectedMethod ); ?>');"
							<?php echo $is_customer_registered && ( $selectedMethod != 'NONE' ) ? "" : " disabled "; ?>>
                            Test : <?php echo esc_html( $testMethod ); ?>
                        </button>
                ...
                <div style="float:right;">
                    <form name="f" method="post" action="" id="mo2f_enable_2FA_on_login_page_form">
                        ...
                        <input type="hidden" name="mo2f_enable_2FA_on_login_page_option_nonce"
                               value="<?php echo esc_attr( wp_create_nonce( "mo2f-enable-2FA-on-login-page-option-nonce" ) ) ?>"/>

                        <input type="checkbox" id="mo2f_enable_2fa_prompt_on_login_page"
                               name="mo2f_enable_2fa_prompt_on_login_page"
                               value="1" <?php checked( molms_Utility::get_mo2f_db_option( 'mo2f_enable_2fa_prompt_on_login_page', 'get_site_option' ) == 1 );

						if ( ! current_user_can( 'administrator' ) && ! in_array(
								$molms_db_queries->get_user_detail( 'mo_2factor_user_registration_status', $user->ID ),
								array(
						...
						<?php echo esc_html( molms_lt( 'Enable 2FA prompt on the WP Login Page' ) ); ?>
                    </form>
                </div>
			...
			echo molms_create_2fa_form( $user, "free_plan", $is_NC ? $free_plan_methods_new_user : $free_plan_methods_existing_user, $can_display_admin_features ); ?>
        </div>

        <hr>
		<?php if ( $can_display_admin_features ) { ?>
            <div>
                   <span id="mo2f_premium_plan"> <a class="mo2f_view_premium_plan_auth_methods"
                                                    onclick="show_premium_auth_methods()">
                        <img src="<?php echo esc_url_raw( plugins_url( 'includes/images/right-arrow.png"', dirname( dirname( __FILE__ ) ) ) ); ?>"
                             class="mo2f_2factor_heading_images"/>
                        <p class="mo2f_heading_style"><?php echo esc_html( molms_lt( 'Premium plan - Authentication methods' ) ); ?>
                                *</p></a></span>
				<?php echo molms_create_2fa_form( $user, "premium_plan", $is_NC ? $premium_plan_methods_new_user : $premium_plan_methods_existing_user ); ?>
    ...
    <div id="EnterEmailCloudVerification" class="modal">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" style="text-align: center; font-size: 20px; color: #20b2aa">Email Address for
                    miniOrange</h3><span id="closeEnterEmailCloud" class="modal-span-close">X</span>
            </div>
            <div class="modal-body" style="height: auto">
                <h2 style="color: red;">The email associated with your account is already registered in miniOrnage.
                    Please Choose another email.</h2>
                <h2><i>Enter your Email:&nbsp;&nbsp;&nbsp; <input type='email' id='emailEnteredCloud'
                                                                  name='emailEnteredCloud' size='40' required
                                                                  value="<?php echo esc_html( $email ); ?>"/></i></h2>
            </div>
            <div class="modal-footer">
                <button type="button" class="mo_lms_button mo_lms_button1 modal-button" id="save_entered_email_cloud">
                    Save
                </button>
            </div>
        </div>
    ...
    <div id="EnterEmail" class="modal">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title" style="text-align: center; font-size: 20px; color: #20b2aa">Email Address for
                    OTP</h3><span id="closeEnterEmail" class="modal-span-close">X</span>
            </div>
            <div class="modal-body" style="height: auto">
                <h2><i>Enter your Email:&nbsp;&nbsp;&nbsp; <input type='email' id='emailEntered' name='emailEntered'
                                                                  size='40' required
                                                                  value="<?php echo esc_html( $email ); ?>"/></i></h2>
            </div>
            <div class="modal-footer">
                <input type="text" id="current_method" hidden value="">
                <button type="button" class="mo_lms_button mo_lms_button1 modal-button" id="save_entered_email">Save
                </button>
            </div>
        </div>
    </div>
    ...
    <div id="mo2f_cloud" class="modal" style="display: none;">
        <div id="mo2f_cloud_modal" class="modal-content" style="width: 30%;overflow: hidden;">
            ...
            <div class="modal-footer">
                <button type="button" class="mo_lms_button mo_lms_button1 modal-button"
                        style="width: 30%;background-color:#61ace5;" id="ConfirmCloudButton1">Confirm
                </button>
                <button type="button" class="mo_lms_button mo_lms_button1 modal-button"
                        style="width: 30%;background-color:#ff4168;" id="closeConfirmCloud1">Cancel
                </button>
    ...
    <script>
        ...
        jQuery('#ConfirmCloudButton1').click(function () {
            ...
            var nonce = '<?php echo esc_attr( wp_create_nonce( "singleUserNonce" ) ); ?>';
            var data = {
            ...
            if (email != '') {
                ...
                jQuery.post(ajaxurl, data, function (response) {
                    ...
                    if (response == "settingsSaved") {
                        ...
                        jQuery('#lms_message').append("<div id='notice_div' class='overlay_success'><div class='popup_text'>&nbsp; &nbsp; An unknown error has occured.</div></div>");
                        window.onload = nav_popup();
                    } else if (response == "USER_LIMIT_EXCEEDED") {
                        jQuery('#EnterEmail').css('display', 'none');
                        jQuery('#lms_message').empty();
                        jQuery('#lms_message').append("<div id='notice_div' class='overlay_error'><div class='popup_text'>&nbsp; &nbsp; Your limit of 3 users has exceeded. Please upgrade to premium plans for more users.</div></div>");
                        window.onload = nav_popup();
                    } else {
                        jQuery('#lms_message').empty()
                        jQuery('#lms_message').append("<div id='notice_div' class='overlay_error'><div class='popup_text'>&nbsp; &nbsp; Invalid Email.</div></div>");
                        window.onload = nav_popup();
                    }
                    close_modal();
                });
        ...
        function configureOrSet2ndFactor_free_plan(authMethod, action, cloudswitch = null, allowed = null) {
			...
			$is_user_registered = $molms_db_queries->get_user_detail( 'mo2f_user_email', $current_user->ID ) ? true : false; ?>
